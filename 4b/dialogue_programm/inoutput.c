...
void greeting(){
    ...
    printf("|4. Output                     |\n");
    printf("|5. Find key                   |\n");
    printf("|6. Find min exceeding key     |\n");
...
void pre_order_output(For_order* pre_order_struct, FILE* file){
    for (unsigned i = 0; i < pre_order_struct->capacity; i++){
        fprintf(file, "{%zu | %s | %zu} -> ", pre_order_struct->array[i].key, pre_order_struct->array[i].info, pre_order_struct->array[i].release);
        if (i != pre_order_struct->capacity-1 && pre_order_struct->array[i].key != pre_order_struct->array[i+1].key)
            printf("\n");
    }
    printf("\n\n");
}
...
void output_data(const Data* data){
    ...
    for (size_t i = 0; i < data->size; i++){
        printf("{%zu | %s | %zu} -> ", 
            ...
            data->storage[i]->info_fieldes.release);
        if (data->storage[i]->duplicate_next == NULL)
            printf("\n");
    }
}
